package net.rsprot.protocol.game.outgoing.codec.camera

import net.rsprot.buffer.JagByteBuf
import net.rsprot.crypto.cipher.StreamCipher
import net.rsprot.protocol.ServerProt
import net.rsprot.protocol.game.outgoing.camera.CamMoveToCyclesV3
import net.rsprot.protocol.game.outgoing.prot.GameServerProt
import net.rsprot.protocol.message.codec.MessageEncoder

public class CamMoveToCyclesV3Encoder : MessageEncoder<CamMoveToCyclesV3> {
    override val prot: ServerProt = GameServerProt.CAM_MOVETO_CYCLES_V3

    override fun encode(
        streamCipher: StreamCipher,
        buffer: JagByteBuf,
        message: CamMoveToCyclesV3,
    ) {
        buffer.p1(if (message.ignoreTerrain) 1 else 0)
        buffer.p2Alt2(message.x)
        buffer.p2Alt1(message.cycles)
        buffer.p1Alt2(message.easing.id)
        buffer.p1Alt1(if (message.heightRelative) 1 else 0)
        buffer.p2(message.z)
        buffer.p2Alt2(message.height)
    }
}
