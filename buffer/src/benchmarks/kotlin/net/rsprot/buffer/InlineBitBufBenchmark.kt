...
@Fork(3)
class InlineBitBufBenchmark {
    private lateinit var byteBuf: ByteBuf
    private lateinit var inlineByteBuf: ByteBuf
    ...
    @Setup
    fun setup() {
        byteBuf = Unpooled.directBuffer(BUFFER_SIZE)
        inlineByteBuf = Unpooled.directBuffer(BUFFER_SIZE + InlineBitBuf.INDEX_PADDING)
    }
    ...
    @Benchmark
    fun largeInlineBitBufferWrite() {
        inlineByteBuf.readerIndex(0)
        inlineByteBuf.writerIndex(0)
        InlineBitBuf.use(inlineByteBuf) {
            for (i in 0..<ITERATION_COUNT) {
                pBits(WRITTEN_BIT_COUNT, 100)
            }
        }
    }
    ...
    private companion object {
        ...
        private const val SMALL_ITERATION_COUNT = 30
    }
}
